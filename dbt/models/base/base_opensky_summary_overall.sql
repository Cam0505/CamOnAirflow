...
popular_destinations as (
    select
        ...
        row_number() over (partition by week, month, year order by count(*) desc) as rn
    from enriched
    group by week, month, year, arrival_airport
),
...
popular_departures as (
    select
        ...
        row_number() over (partition by week, month, year order by count(*) desc) as rn
    from enriched
    group by week, month, year, departure_airport
)
...
from agg_by_period a
left join popular_destinations pd
    on a.week = pd.week and a.month = pd.month and a.year = pd.year and pd.rn = 1
left join popular_departures pdep
    on a.week = pdep.week and a.month = pdep.month and a.year = pdep.year and pdep.rn = 1
"""
dbt Python model: Find all possible ski paths at Cardrona Alpine Resort
"""
    ...
    for seg_id, data in segment_graph.items():
        ...
        for other_id, other_data in segment_graph.items():
            if other_data["from_node"] == to_node:
                ...
                if other_data["run_id"] == data["run_id"] and other_data["segment_idx"] <= data["segment_idx"]:
                    continue
...
def find_paths_from_run(
    ...
    """Breadth-first search to find all possible ski paths from one run."""
    paths = []
    ...
    while queue:
        ...
        if not seg_data["next_segments"]:
            ...
            # compute average gradient as angle (degrees)
            avg_gradient_angle = round(
                math.degrees(math.atan(state["vertical"] / state["distance"])) if state["distance"] > 0 else 0, 1
            )
        ...
        for next_id in seg_data["next_segments"]:
            ...
            new_state = {
                ...
                "distance": state["distance"] + next_data['run_length_m'] / max(next_data['segment_count'], 1),
                "vertical": state["vertical"] + (next_data['top_elevation_m'] - next_data['bottom_elevation_m']) / max(next_data['segment_count'], 1),
                "starting_lift": state["starting_lift"],
                "starting_lift_id": state["starting_lift_id"]
            }
            queue.append((next_id, new_state))
...
def model(dbt, session) -> pd.DataFrame:
    ...
    # find lifts
    lifts_data = lift_run_mapping[lift_run_mapping["connection_type"] == "lift_services_run"].drop_duplicates("lift_osm_id")
    ...
    for _, lift in lifts_data.iterrows():
        ...
        for _, run in lift_runs.iterrows():
            run_paths = find_paths_from_run(segment_graph, ski_segments, ski_runs, lift_run_mapping, run["run_osm_id"], lift_id, lift_name)

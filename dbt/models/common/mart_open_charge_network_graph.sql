...
source_stations AS (
    ...
    FROM driving_distances d
    LEFT JOIN {{ source('opencharge', 'opencharge_stations') }} s
    ON d.id_1 = s.id
),
...
FROM driving_distances d
JOIN source_stations s ON d.id_1 = s.id_1
JOIN target_stations t ON d.id_2 = t.id_2
